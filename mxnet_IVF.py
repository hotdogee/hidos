...
def get_images_format(root_path):
   ...
   for root, _, files in os.walk(root_path):
       root=root.replace('\\','/')
       files=[s for s in files if "*Crop*.jpg" in s]
       if len(files) == 0:
           continue
...
def get_images(root_path):
   ...
   for root, _, files in os.walk(root_path):
       root=root.replace('\\','/')
       files=[s for s in files if ".jpg" in s and str(csize)+"x"+str(csize) not in s]
       if len(files) == 0:
           continue
       fileset = set(files)
       for x in fileset:      
...
random.seed(10)
train_frames = get_images(INPUT_PATH)

# Dump the data of each frame into a CSV file, apply crop to 64 preprocessor
train_lst = write_data_csv(str(INPUT_PATH)+ "/test-"+str(csize)+"x"+str(csize)+"-data.csv", train_frames, lambda x: crop_resize(x, csize))


bs = len(train_frames) # SET HERE

data_val = mx.io.CSVIter(data_csv=str(INPUT_PATH) + "/test-"+str(csize)+"x"+str(csize)+"-data.csv", data_shape=(3, csize, csize),batch_size = bs)
...
for i in range(0,len(train_frames)):
   result[train_frames[i]] = str(Ein_prob[i,0]) 	

with open(str(INPUT_PATH) + "/" + 'predict_result.json', 'w') as f:
   json.dump(result,f)
