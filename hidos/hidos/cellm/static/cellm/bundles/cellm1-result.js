...
$(function () {
  ...
  var path_prefix = '/media/cellm/task/' + task_id + '/' + task_id;
  // check status
    ...
    $.ajax({
      ...
      success: function success(data, status) {
        ...
        if (data.data[0].result_status.toLowerCase() != "success") setTimeout(poll, 3000);else {
          ...
          // display image
          // /media/cellm/task/7ef4f4782fc840738f67a43edafc9683/7ef4f4782fc840738f67a43edafc9683_in.jpg
          $('#input-img').attr('src', '/media/cellm/task/' + task_id + '/' + task_id + '_in.jpg');
          $('#input-download').attr('href', '/media/cellm/task/' + task_id + '/' + task_id + '_in.jpg');
          $('#result-img').attr('src', '/media/cellm/task/' + task_id + '/' + task_id + '_out.jpg');
          $('#result-download').attr('href', '/media/cellm/task/' + task_id + '/' + task_id + '_out.jpg');

          // get image size
          $("<img/>") // Make in memory copy of image to avoid css issues
          .attr("src", $('#input-img').attr("src")).load(function () {
