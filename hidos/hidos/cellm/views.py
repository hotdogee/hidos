...
from .models import CellMAnalysis
from django.http.response import HttpResponse
from django.conf import settings
from os import path, makedirs, chmod
from uuid import uuid4
from .tasks import run_cellm_task
import stat as Perm
import json
import hashlib
...
def status(request):
    ...
    return JsonResponse({
        'data': [{
            'id': t.task_id,
            'result_status': t.result_status,
            } for t in CellMAnalysis.objects.filter(task_id__in=task_ids)]
        })
...
def home(request):
    ...
    if request.method == 'GET':
        ...
        if not CellMAnalysis.objects.filter(task_id=task_id).exists():
            # setup file paths
            #task_id = uuid4().hex # TODO: Create from hash of input to check for duplicate inputs
            path_prefix = path.join(settings.MEDIA_ROOT, 'cellm', 'task', task_id, task_id)
            ...
            run_cellm_task.delay(task_id, args_list, path_prefix)

        # debug
        #run_cellm_task.delay(task_id, args_list, path_prefix).get()
        return HttpResponse(task_id)

def retrieve(request, task_id='1'):
    #return HttpResponse("retrieve = %s." % (task_id))
    try:
        record = CellMAnalysis.objects.get(task_id=task_id)
        return render(
            request,
            'cellm/result.html',
            {
