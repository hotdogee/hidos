...
from .models import CellM2Analysis
from django.http.response import HttpResponse
from django.conf import settings
from os import path, makedirs, chmod
from uuid import uuid4
from .tasks import run_cellm2_task
import stat as Perm
import json
import hashlib
    ...
    if not request.user.is_authenticated():
        ...
        return JsonResponse({
            'data': [{
                ...
                'result_img': settings.MEDIA_URL + 'cellm2/task/' + t.task_id + '/' + t.task_id + '_out.jpg',
                'input_img': settings.MEDIA_URL + 'cellm2/task/' + t.task_id + '/' + t.task_id + '_in.jpg',
                ...
                } for t in CellM2Analysis.objects.filter(user=request.user)]
            })
...
def status(request):
    ...
    return JsonResponse({
        'data': [{
            ...
            'result_outerr': json.loads(t.result_outerr or '{}'),
            'result_status': t.result_status,
            } for t in CellM2Analysis.objects.filter(task_id__in=task_ids)]
        })
...
def home(request):
    ...
    if request.method == 'GET':
        ...
        if not CellM2Analysis.objects.filter(task_id=task_id).exists():
            logger.info('New task_id: {0}'.format(task_id))
            # setup file paths
            #task_id = uuid4().hex # TODO: Create from hash of input to check for duplicate inputs
            path_prefix = path.join(settings.MEDIA_ROOT, 'cellm2', 'task', task_id, task_id)
            ...
            run_cellm2_task.delay(task_id, args_list, path_prefix)
        else:
            logger.info('Duplicate task_id: {0}'.format(task_id))
            
        # debug
        #run_cellm2_task.delay(task_id, args_list, path_prefix).get()
        return HttpResponse(task_id)

def retrieve(request, task_id='1'):
    #return HttpResponse("retrieve = %s." % (task_id))
    try:
        record = CellM2Analysis.objects.get(task_id=task_id)
        return render(
            request,
            'cellm2/result.html',
            {
                'title': 'Cell M2 - {0}'.format(record.user_filename),
                'year': datetime.now().year,
