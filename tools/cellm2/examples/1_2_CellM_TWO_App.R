...
library("RJSONIO")
  ...
  hist_save <- hist(c(img_tif_rs),breaks=100,plot= FALSE)
  ...
  for(inx in InxBegin:-1:1){  
    ...
    if ((GrowthOfHist[inx] < min_value) & (nextGrowthRate > 0.2)){
  ...
  img_processed = paintObjects(contour_pattern, channel(img_tif_rs_intensity, 'rgb'), col='#ffff00')
