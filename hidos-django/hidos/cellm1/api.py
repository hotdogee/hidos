...
from .serializers import CellM1TaskSerializer
...
class TaskViewSet(viewsets.ModelViewSet):
     ...
     * move(destination)
    """
    queryset = CellM1Task.objects.all()
    serializer_class = CellM1TaskSerializer
    ...
    def get_queryset(self):
        ...
        if user.username:
            ...
            if user.is_superuser:
                return CellM1Task.objects.all()
            else:
                return CellM1Task.objects.filter(user=self.request.user)
        else:
            return CellM1Task.objects.none()
    ...
    def running(self, request, *args, **kwargs):
        running_tasks = CellM1Task.objects.filter(user=request.user, status__in=['queued', 'running'])
