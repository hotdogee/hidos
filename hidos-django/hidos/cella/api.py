...
from .serializers import CellATaskSerializer
...
class TaskViewSet(viewsets.ModelViewSet):
     ...
     * move(destination)
    """
    queryset = CellATask.objects.all()
    serializer_class = CellATaskSerializer
    ...
    def get_queryset(self):
        ...
        if user.username:
            ...
            if user.is_superuser:
                return CellATask.objects.all()
            else:
                return CellATask.objects.filter(user=self.request.user)
        else:
            return CellATask.objects.none()
    ...
    def running(self, request, *args, **kwargs):
        running_tasks = CellATask.objects.filter(user=request.user, status__in=['queued', 'running'])
