...
from .tasks import run_cell_m3_task
...
class CellM3Task(CellTaskModel):
    ...
    def get_absolute_url(self):
        return reverse('m3:detail', kwargs={'task_id': self.task_id}, current_app=app_name)
    ...
    def enqueue(self):
        ...
        path_prefix = path.join(settings.MEDIA_ROOT, 'cellm3', 'task', self.task_id, self.task_id)
        script_path = path.join(settings.PROJECT_ROOT, 'cellm3', 'bin', '1_6_obj_area_cal_cmd.R')
        ...
        # build command
        run_cell_m3_task.delay(self.task_id, input_image_viewer_path, result_image_path, result_json_path, path_prefix)
        print(uploaded_image_path, result_image_path, result_json_path)

    def original_image(self):
        return format_html('<a href="/media/cellm3/task/{}/{}_in.jpg">Uploaded Image</a>', self.task_id,
                           self.task_id)
