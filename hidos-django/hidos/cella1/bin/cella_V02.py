...
def findExtremities(img):
    ...
    # locate endpoint regions
    b1 = m.union(m.supcanon(img, hmt1), m.supcanon(
        img, hmt2), m.supcanon(img, hmt3))
...
def cellAngiogenesis(task_record, add_boarder=False):
    max_size = 1536
    # load image
...
#    io.show()
    ...
    img_dilation = cv2.morphologyEx(
        np.uint8(img_dilation), cv2.MORPH_CLOSE, kernel)
#    plt.figure()
#    io.imshow(img_dilation)
#    io.show()
...
#    io.show()
    ...
    # the area of maximum hole still too small, therefore remove all holes
    if float(hole_size[-1]) / img_fill_holes.size < 0.005:
        size_thresh = hole_size[-1] + 1000
    else:
        area_thresh = (img_ori_resize.shape[
                       0] / 30) * (img_ori_resize.shape[1] / 30)
...
#    print(size_thresh,index,numpy.size(hole_size))
    ...
    img_remove_small_hole = morphology.remove_small_holes(
        img_skeleton, min_size=size_thresh)
#    plt.figure()
#    io.imshow(img_remove_small_hole)
#    io.show()

    img_skeleton = morphology.skeletonize(
        img_remove_small_hole)  # remove small hole by skeleton
#    plt.figure()
#    io.imshow(img_skeleton)
#    io.show()
...
#        io.show()
        ...
        small_branch = deleteBranch(
            img_remove_isolatedPoints, extremityMap, junctionMap, 30)
...
#        io.show()
        ...
        img_skeleton = m.thin(img_remove_small_object,
                              m.endpoints('homotopic'), 1)
...
#    io.show()
    ...
    img_remove_small_hole = morphology.remove_small_holes(
        img_final, min_size=size_thresh)
    img_final = morphology.skeletonize(
        img_remove_small_hole)  # remove small hole by skeleton
#    plt.figure()
#    io.imshow(img_final)
#    io.show()
...
#    io.show()
    ...
    allBranch, total_num_of_branch = detectAllBranch(
        img_final, extremityMap, junctionMap)
...
#    io.show()
    ...
    ret_seg, img_label_seg, stats_seg, centroids_seg = cv2.connectedComponentsWithStats(
        np.uint8(allSegment))
...
#    io.show()
    ...
    ret_branch, img_label_branch, stats_branch, centroids_branch = cv2.connectedComponentsWithStats(
        np.uint8(allBranch))
    ...
    ret_mesh, img_label_mesh, stats_mesh, centroids_mesh = cv2.connectedComponentsWithStats(
        img_region)
    num_mesh = ret_mesh - 1
...
#    io.show()
    ...
    ret_perimeter, img_label_perimeter, stats_perimeter, centroids_perimeter = cv2.connectedComponentsWithStats(
        np.uint8(img_perimeter))
    ...
    img_ori_resize[mask0] = (img_ori_resize[mask0] * 0.7) + \
        (img_label_color[mask0] * 0.3)
    img_ori_resize[mask1] = (img_ori_resize[mask1] * 0.7) + \
        (img_label_color[mask1] * 0.3)
    img_ori_resize[mask2] = (img_ori_resize[mask2] * 0.7) + \
        (img_label_color[mask2] * 0.3)
...
#    io.show()
    ...
    for i in range(1, ret_mesh):
        cv2.putText(img_ori_resize, str(i), (int(centroids_mesh[i, 0]), int(
            centroids_mesh[i, 1])), font, 1, (255, 0, 0), 2)
