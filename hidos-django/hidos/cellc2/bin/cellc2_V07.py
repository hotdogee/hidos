...
#    print (tmp1,tmp2,float(tmp2)/tmp1)
    if float(tmp2)/tmp1<2:  #假如比例增加太多，就不執行constrast saturated
    ...
    for i in range(1,ret):
        if (stats[i,4]<900):
            if (centroids[i,1]-radius>0 and centroids[i,1]+radius<img_tmp.shape[0] and centroids[i,0]-radius>0 and centroids[i,0]+radius<img_tmp.shape[1]):
                tmp1=max(centroids[i,1]-radius,0)
                tmp2=min(centroids[i,1]+radius+1,img_tmp.shape[0])
                tmp3=max(centroids[i,0]-radius,0)
                tmp4=min(centroids[i,0]+radius+1,img_tmp.shape[1])             
...
##img_input_path=u'D:\\Aaron workspace\\Aaron\\CellConfluence_Project\\tim_20161019'
##img_input_path=u'D:\\Aaron workspace\\Aaron\\CellConfluence_Project\\tim_wugu'
