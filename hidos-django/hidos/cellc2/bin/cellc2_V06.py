...
#    mask=np.where(img_tmp==1)
#    backgroud_mean=np.mean(img_gray[mask])
#    backgroud_std=np.std(img_gray[mask])
##    print ('%f,%f',[backgroud_mean, backgroud_std])
#    mask=np.where(img_gray<=backgroud_mean-1.8*backgroud_std)
#    img_opening[mask]=1
##    img_opening=cv2.dilate(img_opening,kernel,iterations=1)
###    img_opening=cv2.erode(img_opening,kernel,ite2rations=1)
##    
##    if (float(img_opening.sum())/img_opening.size>0.8):
##        img_opening=cv2.erode(img_opening,kernel,iterations=1)
##    else:
...
#                    io.show()                            
    ...
    if (float(img_opening.sum())/img_opening.size>0.8):
        ...
        img_opening=cv2.erode(np.uint8(img_opening),kernel)   
#        img_opening=cv2.erode(np.uint8(img_opening),kernel3,iterations=2)        
